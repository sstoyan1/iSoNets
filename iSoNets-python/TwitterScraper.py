...
__author__ = 'Tom Dickinson, Slavina Stoyanova'
...
#-----------------------------------------------------------------------
secrets = {}
exec(compile(open('secrets.py',"rb").read(), 'secrets.py','exec'), secrets)
    ...
    def save_tweets(self, tweets):
        ...
        for tweet in tweets:
            ...
            if tweet['created_at'] is not None:
                ...
                try:
                    tweet_json = twapi.get_status('913824993359917056')
                    print ("%s" % tweet_json)
                    #print('%s,%s' % (tweet_id, tweet.text.encode('UTF-8')))
                except tweepy.TweepError as te:
                    print('Failed to get tweet ID %s'% tweet['tweet_id'])
...
if __name__ == '__main__':
    ...
    select_tweets_since = datetime.datetime.strptime("2017-08-17", '%Y-%m-%d')
    select_tweets_until = datetime.datetime.strptime("2017-09-03", '%Y-%m-%d')
